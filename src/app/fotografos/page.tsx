...
export default function PhotographerDashboardPage() {
  ...
  async function createEvent(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (res.ok) {
      ...
      setStatusOk(true);
      setStatus(`Evento creado → /eventos/${data.slug}`);
    } else {
      setStatusOk(false);
  ...
  async function uploadPhotos(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (ok > 0 && autoAnalyze) {
      ...
      while (remaining > 0 && rounds < 200) {
        const ar = await fetch("/api/photographer/analyze-event", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        <div className="grid gap-6 xl:grid-cols-12">
          ...
          <div className="space-y-6 xl:col-span-5">
            ...
            <AdminCard
              step="2"
              title="Nuevo evento"
              description="Creá el evento y publicalo para que aparezca en el sitio."
            >
              <form onSubmit={createEvent} className="space-y-4">
                ...
                <label className="block text-sm">
                  ...
                  <select
                    ...
                    className="mt-1 w-full rounded-[var(--radius)] border border-[var(--border)] bg-[var(--bg)] px-3 py-2 outline-none transition focus:border-[var(--accent)]"
                  >
                    {SPORT_OPTIONS.map((s) => (
